/**
 * English translations for Sinopia
 * 
 * Structure:
 * - common: Shared strings across the app
 * - nav: Navigation labels
 * - home: Homepage content
 * - auth: Authentication pages
 * - dashboard: Dashboard and protected pages
 * - footer: Footer content
 * 
 * To add a new key: Add it here and in de.ts with the German translation
 */
export const en = {
  common: {
    appName: "Sinopia",
    loading: "Loading...",
    save: "Save",
    cancel: "Cancel",
    submit: "Submit",
    delete: "Delete",
    edit: "Edit",
    view: "View",
    close: "Close",
    back: "Back",
    next: "Next",
    search: "Search",
    filter: "Filter",
    noResults: "No results found",
    error: "An error occurred",
    success: "Success",
    language: "Language",
    switchToGerman: "Switch to German",
    switchToEnglish: "Switch to English",
  },
  nav: {
    home: "Home",
    about: "About",
    contact: "Contact",
    signIn: "Sign In",
    signUp: "Sign Up",
    getStarted: "Get Started",
    dashboard: "Dashboard",
    projects: "Projects",
    offers: "Offers",
    contracts: "Contracts",
    payments: "Payments",
    notifications: "Notifications",
    profile: "Profile",
    signOut: "Sign Out",
  },
  home: {
    heroTitle: "Connect Your Skills with",
    heroTitleHighlight: "Opportunities",
    heroSubtitle: "Sinopia enables professionals to present their skills to companies seeking qualified talent. Find your next project or hire the perfect freelancer today.",
    learnMore: "Learn More",
    trustedBy: "Trusted by 10,000+ professionals",
    activeUsers: "Active Users",
    projectsCompleted: "Projects Completed",
    averageRating: "Average Rating",
    featuresTitle: "Why Choose Sinopia?",
    featuresSubtitle: "Everything you need to connect skills with opportunities",
    feature1Title: "Find Perfect Matches",
    feature1Desc: "Our smart matching algorithm connects you with the right projects or talents based on your skills and requirements.",
    feature2Title: "Secure Payments",
    feature2Desc: "Protected transactions with milestone-based payments ensure both parties are satisfied before funds are released.",
    feature3Title: "Professional Contracts",
    feature3Desc: "Legally binding digital contracts that protect both skill givers and searchers throughout the project.",
    ctaTitle: "Ready to Get Started?",
    ctaSubtitle: "Join thousands of professionals already using Sinopia",
    ctaButton: "Create Your Account",
  },
  auth: {
    signInTitle: "Welcome Back",
    signInSubtitle: "Sign in to your Sinopia account",
    signUpTitle: "Create Account",
    signUpSubtitle: "Join Sinopia and start your journey",
    email: "Email",
    emailPlaceholder: "Enter your email",
    password: "Password",
    passwordPlaceholder: "Enter your password",
    confirmPassword: "Confirm Password",
    confirmPasswordPlaceholder: "Confirm your password",
    firstName: "First Name",
    lastName: "Last Name",
    forgotPassword: "Forgot Password?",
    rememberMe: "Remember me",
    noAccount: "Don't have an account?",
    hasAccount: "Already have an account?",
    cvRequired: "Please upload your CV to continue.",
    signInButton: "Sign In",
    signUpButton: "Create Account",
    orContinueWith: "Or continue with",
    skillGiver: "Skill Giver",
    skillSearcher: "Skill Searcher",
    skillGiverDesc: "I want to offer my skills and find projects",
    skillSearcherDesc: "I want to find talented professionals",
    selectRole: "Select your role",
    forgotPasswordTitle: "Forgot Password",
    forgotPasswordSubtitle: "Enter your email to receive a reset code",
    sendCode: "Send Reset Code",
    resetPasswordTitle: "Reset Password",
    resetPasswordSubtitle: "Enter your new password",
    resetButton: "Reset Password",
    otpTitle: "Verify Your Email",
    otpSubtitle: "Enter the 6-digit code sent to your email",
    verifyButton: "Verify",
    resendCode: "Resend Code",
  },
  dashboard: {
    welcome: "Welcome back",
    welcomeName: "Welcome back, {name}!",
    overview: "Overview",
    recentProjects: "Recent Projects",
    pendingOffers: "Pending Offers",
    activeContracts: "Active Contracts",
    activeProjectsCount: "Active Projects",
    totalEarnings: "Total Earnings",
    totalSpent: "Total Spent",
    completedProjects: "Completed Projects",
    pendingPayments: "Pending Payments",
    quickActions: "Quick Actions",
    viewAll: "View All",
    noProjects: "No projects yet",
    noActiveProjects: "No active projects",
    noOffers: "No pending offers",
    noContracts: "No active contracts",
    whatsHappeningGiver: "Here's what's happening with your projects today.",
    whatsHappeningSearcher: "Here's what's happening with your team today.",
    findProjects: "Find Projects",
    postProject: "Post Project",
    progress: "Progress",
    from: "From",
  },
  projects: {
    title: "Projects",
    addNew: "Add New Project",
    newProject: "New Project",
    createProject: "Create Project",
    search: "Search projects...",
    status: "Status",
    allStatus: "All Status",
    budget: "Budget",
    deadline: "Deadline",
    due: "Due",
    skills: "Skills Required",
    description: "Description",
    viewDetails: "View Details",
    applyNow: "Apply Now",
    statusDraft: "Draft",
    statusOpen: "Open",
    statusInProgress: "In Progress",
    statusCompleted: "Completed",
    statusCancelled: "Cancelled",
    noProjectsFound: "No projects found",
    adjustFilters: "Try adjusting your search or filters",
    browseProjects: "Browse available projects to get started",
    createFirst: "Create your first project to find talent",
    manageGiver: "View and manage your assigned projects",
    manageSearcher: "Manage your posted projects",
    stages: "stages",
    moreSkills: "more",
    progress: "Progress",
    filterByStatus: "Filter by status",
  },
  offers: {
    title: "Offers",
    pending: "Pending",
    accepted: "Accepted",
    rejected: "Rejected",
    amount: "Offer Amount",
    message: "Message",
    accept: "Accept",
    reject: "Reject",
    withdraw: "Withdraw",
    search: "Search offers...",
    allStatus: "All Status",
    noOffersFound: "No offers found",
    noOffersToDisplay: "No offers to display at the moment",
    adjustFilters: "Try adjusting your search or filters",
    manageGiver: "View offers you have sent",
    manageSearcher: "Review and manage incoming offers",
    sent: "Sent",
    acceptOffer: "Accept Offer",
    rejectOffer: "Reject Offer",
    confirmAccept: "Are you sure you want to accept this offer? A contract will be created.",
    confirmReject: "Are you sure you want to reject this offer? This action cannot be undone.",
    offerAccepted: "Offer accepted!",
    contractCreated: "The contract will be created shortly.",
    offerRejected: "Offer rejected",
    offerDeclined: "The offer has been declined.",
    filterByStatus: "Filter by status",
  },
  contracts: {
    title: "Contracts",
    sign: "Sign Contract",
    signed: "Signed",
    pending: "Pending Signature",
    viewContract: "View Contract",
    downloadPdf: "Download PDF",
    search: "Search contracts...",
    allStatus: "All Status",
    noContractsFound: "No contracts found",
    noContractsToDisplay: "No contracts to display at the moment",
    adjustFilters: "Try adjusting your search or filters",
    manageGiver: "View and manage your contracts",
    manageSearcher: "Review and manage your contracts",
    contractValue: "Contract Value",
    signedOn: "Signed on",
    awaitingSignature: "Awaiting signature",
    filterByStatus: "Filter by status",
    active: "Active",
  },
  payments: {
    title: "Payments",
    invoices: "Invoices",
    pending: "Pending",
    paid: "Paid",
    overdue: "Overdue",
    amount: "Amount",
    dueDate: "Due",
    payNow: "Pay Now",
    downloadInvoice: "Download Invoice",
    search: "Search invoices...",
    allStatus: "All Status",
    noPaymentsFound: "No invoices found",
    noPaymentsToDisplay: "No invoices to display at the moment",
    adjustFilters: "Try adjusting your search or filters",
    manageGiver: "Track your invoices and earnings",
    manageSearcher: "Manage your payments and invoices",
    filterByStatus: "Filter by status",
    totalEarned: "Total Earned",
    totalPaid: "Total Paid",
    paymentHistory: "Payment History",
    noPaymentsYet: "No payments yet",
    paymentHistoryWillAppear: "Payment history will appear here",
    recentPayments: "Recent Payments",
  },
  profile: {
    title: "Profile",
    editProfile: "Edit Profile",
    personalInfo: "Personal Information",
    professionalInfo: "Professional Information",
    skills: "Skills",
    portfolio: "Portfolio",
    reviews: "Reviews",
    settings: "Settings",
    uploadPhoto: "Upload Photo",
    bio: "Bio",
    about: "About",
    hourlyRate: "Hourly Rate",
    availability: "Availability",
    location: "Location",
    saveChanges: "Save Changes",
    manageProfile: "Manage your profile",
    manageProfessionalProfile: "Manage your professional profile",
    manageCompanyProfile: "Manage your company profile",
    addSkill: "Add a skill",
    removeSkill: "Remove",
    experience: "Experience",
    education: "Education",
    certifications: "Certifications",
    present: "Present",
    profileUpdated: "Your changes have been saved successfully.",
  },
  footer: {
    skillGiver: "Skill Giver",
    findProjects: "Find Projects",
    buildPortfolio: "Build Portfolio",
    getPaid: "Get Paid",
    skillSearcher: "Skill Searcher",
    postProjects: "Post Projects",
    findTalent: "Find Talent",
    manageTeams: "Manage Teams",
    legal: "Legal",
    terms: "Terms of Service",
    privacy: "Privacy Policy",
    contactUs: "Contact Us",
    copyright: "All rights reserved.",
    tagline: "Connect skills with opportunities. Build your career or find the talent you need.",
  },
  about: {
    title: "About Sinopia",
    subtitle: "Connecting talent with opportunity since 2024",
    missionTitle: "Our Mission",
    missionText: "We believe that every professional deserves the opportunity to showcase their skills and connect with companies that value their expertise.",
    visionTitle: "Our Vision",
    visionText: "To become the world's most trusted platform for skill-based professional connections.",
  },
  contact: {
    title: "Contact Us",
    subtitle: "We'd love to hear from you",
    nameLabel: "Your Name",
    emailLabel: "Email Address",
    subjectLabel: "Subject",
    messageLabel: "Message",
    sendButton: "Send Message",
    successMessage: "Thank you for your message. We'll get back to you soon!",
  },
};

export type TranslationKeys = typeof en;

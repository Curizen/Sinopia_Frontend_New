/**
 * German translations for Sinopia
 * 
 * Struktur:
 * - common: Gemeinsame Texte in der App
 * - nav: Navigationsbeschriftungen
 * - home: Startseiten-Inhalt
 * - auth: Authentifizierungsseiten
 * - dashboard: Dashboard und geschützte Seiten
 * - footer: Fußzeilen-Inhalt
 */
export const de = {
  common: {
    appName: "Sinopia",
    loading: "Laden...",
    save: "Speichern",
    cancel: "Abbrechen",
    submit: "Absenden",
    delete: "Löschen",
    edit: "Bearbeiten",
    view: "Ansehen",
    close: "Schließen",
    back: "Zurück",
    next: "Weiter",
    search: "Suchen",
    filter: "Filtern",
    noResults: "Keine Ergebnisse gefunden",
    error: "Ein Fehler ist aufgetreten",
    success: "Erfolgreich",
    language: "Sprache",
    switchToGerman: "Zu Deutsch wechseln",
    switchToEnglish: "Zu Englisch wechseln",
  },
  nav: {
    home: "Startseite",
    about: "Über uns",
    contact: "Kontakt",
    signIn: "Anmelden",
    signUp: "Registrieren",
    getStarted: "Jetzt starten",
    dashboard: "Dashboard",
    projects: "Projekte",
    offers: "Angebote",
    contracts: "Verträge",
    payments: "Zahlungen",
    notifications: "Benachrichtigungen",
    profile: "Profil",
    signOut: "Abmelden",
  },
  home: {
    heroTitle: "Verbinde deine Fähigkeiten mit",
    heroTitleHighlight: "Möglichkeiten",
    heroSubtitle: "Sinopia ermöglicht es Fachleuten, ihre Fähigkeiten Unternehmen zu präsentieren, die qualifizierte Talente suchen. Finde dein nächstes Projekt oder stelle den perfekten Freelancer ein.",
    learnMore: "Mehr erfahren",
    trustedBy: "Vertraut von 10.000+ Fachleuten",
    activeUsers: "Aktive Nutzer",
    projectsCompleted: "Abgeschlossene Projekte",
    averageRating: "Durchschnittliche Bewertung",
    featuresTitle: "Warum Sinopia wählen?",
    featuresSubtitle: "Alles, was Sie brauchen, um Fähigkeiten mit Möglichkeiten zu verbinden",
    feature1Title: "Perfekte Übereinstimmungen finden",
    feature1Desc: "Unser intelligenter Matching-Algorithmus verbindet Sie mit den richtigen Projekten oder Talenten basierend auf Ihren Fähigkeiten und Anforderungen.",
    feature2Title: "Sichere Zahlungen",
    feature2Desc: "Geschützte Transaktionen mit meilensteinbasierten Zahlungen stellen sicher, dass beide Parteien zufrieden sind, bevor Gelder freigegeben werden.",
    feature3Title: "Professionelle Verträge",
    feature3Desc: "Rechtlich bindende digitale Verträge, die sowohl Fähigkeitengeber als auch -sucher während des gesamten Projekts schützen.",
    ctaTitle: "Bereit anzufangen?",
    ctaSubtitle: "Schließen Sie sich Tausenden von Fachleuten an, die Sinopia bereits nutzen",
    ctaButton: "Konto erstellen",
  },
  auth: {
    signInTitle: "Willkommen zurück",
    signInSubtitle: "Melden Sie sich bei Ihrem Sinopia-Konto an",
    signUpTitle: "Konto erstellen",
    signUpSubtitle: "Treten Sie Sinopia bei und starten Sie Ihre Reise",
    email: "E-Mail",
    emailPlaceholder: "Geben Sie Ihre E-Mail ein",
    password: "Passwort",
    passwordPlaceholder: "Geben Sie Ihr Passwort ein",
    confirmPassword: "Passwort bestätigen",
    confirmPasswordPlaceholder: "Bestätigen Sie Ihr Passwort",
    firstName: "Vorname",
    lastName: "Nachname",
    forgotPassword: "Passwort vergessen?",
    rememberMe: "Angemeldet bleiben",
    noAccount: "Noch kein Konto?",
    hasAccount: "Bereits ein Konto?",
    cvRequired: "Bitte laden Sie Ihren Lebenslauf hoch, um fortzufahren.",
    signInButton: "Anmelden",
    signUpButton: "Konto erstellen",
    orContinueWith: "Oder fortfahren mit",
    skillGiver: "Fähigkeitengeber",
    skillSearcher: "Fähigkeitensucher",
    skillGiverDesc: "Ich möchte meine Fähigkeiten anbieten und Projekte finden",
    skillSearcherDesc: "Ich möchte talentierte Fachleute finden",
    selectRole: "Wählen Sie Ihre Rolle",
    forgotPasswordTitle: "Passwort vergessen",
    forgotPasswordSubtitle: "Geben Sie Ihre E-Mail ein, um einen Reset-Code zu erhalten",
    sendCode: "Reset-Code senden",
    resetPasswordTitle: "Passwort zurücksetzen",
    resetPasswordSubtitle: "Geben Sie Ihr neues Passwort ein",
    resetButton: "Passwort zurücksetzen",
    otpTitle: "E-Mail bestätigen",
    otpSubtitle: "Geben Sie den 6-stelligen Code ein, der an Ihre E-Mail gesendet wurde",
    verifyButton: "Bestätigen",
    resendCode: "Code erneut senden",
  },
  dashboard: {
    welcome: "Willkommen zurück",
    welcomeName: "Willkommen zurück, {name}!",
    overview: "Übersicht",
    recentProjects: "Aktuelle Projekte",
    pendingOffers: "Ausstehende Angebote",
    activeContracts: "Aktive Verträge",
    activeProjectsCount: "Aktive Projekte",
    totalEarnings: "Gesamteinnahmen",
    totalSpent: "Gesamtausgaben",
    completedProjects: "Abgeschlossene Projekte",
    pendingPayments: "Ausstehende Zahlungen",
    quickActions: "Schnellaktionen",
    viewAll: "Alle anzeigen",
    noProjects: "Noch keine Projekte",
    noActiveProjects: "Keine aktiven Projekte",
    noOffers: "Keine ausstehenden Angebote",
    noContracts: "Keine aktiven Verträge",
    whatsHappeningGiver: "Das passiert heute mit Ihren Projekten.",
    whatsHappeningSearcher: "Das passiert heute mit Ihrem Team.",
    findProjects: "Projekte finden",
    postProject: "Projekt erstellen",
    progress: "Fortschritt",
    from: "Von",
  },
  projects: {
    title: "Projekte",
    addNew: "Neues Projekt hinzufügen",
    newProject: "Neues Projekt",
    createProject: "Projekt erstellen",
    search: "Projekte suchen...",
    status: "Status",
    allStatus: "Alle Status",
    budget: "Budget",
    deadline: "Frist",
    due: "Fällig",
    skills: "Erforderliche Fähigkeiten",
    description: "Beschreibung",
    viewDetails: "Details anzeigen",
    applyNow: "Jetzt bewerben",
    statusDraft: "Entwurf",
    statusOpen: "Offen",
    statusInProgress: "In Bearbeitung",
    statusCompleted: "Abgeschlossen",
    statusCancelled: "Storniert",
    noProjectsFound: "Keine Projekte gefunden",
    adjustFilters: "Versuchen Sie, Ihre Suche oder Filter anzupassen",
    browseProjects: "Durchsuchen Sie verfügbare Projekte, um zu beginnen",
    createFirst: "Erstellen Sie Ihr erstes Projekt, um Talente zu finden",
    manageGiver: "Anzeigen und verwalten Sie Ihre zugewiesenen Projekte",
    manageSearcher: "Verwalten Sie Ihre veröffentlichten Projekte",
    stages: "Phasen",
    moreSkills: "weitere",
    progress: "Fortschritt",
    filterByStatus: "Nach Status filtern",
  },
  offers: {
    title: "Angebote",
    pending: "Ausstehend",
    accepted: "Angenommen",
    rejected: "Abgelehnt",
    amount: "Angebotsbetrag",
    message: "Nachricht",
    accept: "Annehmen",
    reject: "Ablehnen",
    withdraw: "Zurückziehen",
    search: "Angebote suchen...",
    allStatus: "Alle Status",
    noOffersFound: "Keine Angebote gefunden",
    noOffersToDisplay: "Derzeit keine Angebote anzuzeigen",
    adjustFilters: "Versuchen Sie, Ihre Suche oder Filter anzupassen",
    manageGiver: "Angebote anzeigen, die Sie gesendet haben",
    manageSearcher: "Eingehende Angebote prüfen und verwalten",
    sent: "Gesendet",
    acceptOffer: "Angebot annehmen",
    rejectOffer: "Angebot ablehnen",
    confirmAccept: "Sind Sie sicher, dass Sie dieses Angebot annehmen möchten? Ein Vertrag wird erstellt.",
    confirmReject: "Sind Sie sicher, dass Sie dieses Angebot ablehnen möchten? Diese Aktion kann nicht rückgängig gemacht werden.",
    offerAccepted: "Angebot angenommen!",
    contractCreated: "Der Vertrag wird in Kürze erstellt.",
    offerRejected: "Angebot abgelehnt",
    offerDeclined: "Das Angebot wurde abgelehnt.",
    filterByStatus: "Nach Status filtern",
  },
  contracts: {
    title: "Verträge",
    sign: "Vertrag unterzeichnen",
    signed: "Unterzeichnet",
    pending: "Unterschrift ausstehend",
    viewContract: "Vertrag anzeigen",
    downloadPdf: "PDF herunterladen",
    search: "Verträge suchen...",
    allStatus: "Alle Status",
    noContractsFound: "Keine Verträge gefunden",
    noContractsToDisplay: "Derzeit keine Verträge anzuzeigen",
    adjustFilters: "Versuchen Sie, Ihre Suche oder Filter anzupassen",
    manageGiver: "Ihre Verträge anzeigen und verwalten",
    manageSearcher: "Ihre Verträge prüfen und verwalten",
    contractValue: "Vertragswert",
    signedOn: "Unterzeichnet am",
    awaitingSignature: "Wartet auf Unterschrift",
    filterByStatus: "Nach Status filtern",
    active: "Aktiv",
  },
  payments: {
    title: "Zahlungen",
    invoices: "Rechnungen",
    pending: "Ausstehend",
    paid: "Bezahlt",
    overdue: "Überfällig",
    amount: "Betrag",
    dueDate: "Fällig",
    payNow: "Jetzt bezahlen",
    downloadInvoice: "Rechnung herunterladen",
    search: "Rechnungen suchen...",
    allStatus: "Alle Status",
    noPaymentsFound: "Keine Rechnungen gefunden",
    noPaymentsToDisplay: "Derzeit keine Rechnungen anzuzeigen",
    adjustFilters: "Versuchen Sie, Ihre Suche oder Filter anzupassen",
    manageGiver: "Ihre Rechnungen und Einnahmen verfolgen",
    manageSearcher: "Ihre Zahlungen und Rechnungen verwalten",
    filterByStatus: "Nach Status filtern",
    totalEarned: "Gesamteinnahmen",
    totalPaid: "Insgesamt bezahlt",
    paymentHistory: "Zahlungsverlauf",
    noPaymentsYet: "Noch keine Zahlungen",
    paymentHistoryWillAppear: "Der Zahlungsverlauf wird hier angezeigt",
    recentPayments: "Aktuelle Zahlungen",
  },
  profile: {
    title: "Profil",
    editProfile: "Profil bearbeiten",
    personalInfo: "Persönliche Informationen",
    professionalInfo: "Berufliche Informationen",
    skills: "Fähigkeiten",
    portfolio: "Portfolio",
    reviews: "Bewertungen",
    settings: "Einstellungen",
    uploadPhoto: "Foto hochladen",
    bio: "Biografie",
    about: "Über mich",
    hourlyRate: "Stundensatz",
    availability: "Verfügbarkeit",
    location: "Standort",
    saveChanges: "Änderungen speichern",
    manageProfile: "Verwalten Sie Ihr Profil",
    manageProfessionalProfile: "Verwalten Sie Ihr berufliches Profil",
    manageCompanyProfile: "Verwalten Sie Ihr Unternehmensprofil",
    addSkill: "Fähigkeit hinzufügen",
    removeSkill: "Entfernen",
    experience: "Berufserfahrung",
    education: "Ausbildung",
    certifications: "Zertifizierungen",
    present: "Aktuell",
    profileUpdated: "Ihre Änderungen wurden erfolgreich gespeichert.",
  },
  footer: {
    skillGiver: "Fähigkeitengeber",
    findProjects: "Projekte finden",
    buildPortfolio: "Portfolio aufbauen",
    getPaid: "Bezahlt werden",
    skillSearcher: "Fähigkeitensucher",
    postProjects: "Projekte veröffentlichen",
    findTalent: "Talente finden",
    manageTeams: "Teams verwalten",
    legal: "Rechtliches",
    terms: "Nutzungsbedingungen",
    privacy: "Datenschutzrichtlinie",
    contactUs: "Kontakt",
    copyright: "Alle Rechte vorbehalten.",
    tagline: "Verbinde Fähigkeiten mit Möglichkeiten. Baue deine Karriere auf oder finde die Talente, die du brauchst.",
  },
  about: {
    title: "Über Sinopia",
    subtitle: "Verbindung von Talent mit Möglichkeiten seit 2024",
    missionTitle: "Unsere Mission",
    missionText: "Wir glauben, dass jeder Fachmann die Möglichkeit verdient, seine Fähigkeiten zu präsentieren und sich mit Unternehmen zu verbinden, die seine Expertise schätzen.",
    visionTitle: "Unsere Vision",
    visionText: "Die weltweit vertrauenswürdigste Plattform für fähigkeitsbasierte berufliche Verbindungen zu werden.",
  },
  contact: {
    title: "Kontakt",
    subtitle: "Wir freuen uns von Ihnen zu hören",
    nameLabel: "Ihr Name",
    emailLabel: "E-Mail-Adresse",
    subjectLabel: "Betreff",
    messageLabel: "Nachricht",
    sendButton: "Nachricht senden",
    successMessage: "Vielen Dank für Ihre Nachricht. Wir melden uns bald bei Ihnen!",
  },
};
